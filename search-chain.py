...
from scores import ideal, npd, cutedges
import pandas as pd
import os
...
for id in run["configurations"]:
	...
	pi = ideals[id]
	chain.steps = 50000
	acceptance = ideal(pi, "hcvap%", chain.steps, run["L"], run["k"], run["phi"])
	chain.accept = Accept.mh(acceptance)
	...
	for partition in chain.progress():
		...
		chain.collect(
			partition,
			updaters=[
				"hcvap%", "bcvap%", "distance", "accept", "cutedges", "centers"
			]
		)
	...
	if not ex(p.join(root, id, "guidance")): os.mkdir(p.join(root, id, "guidance"))
	root = p.join(root, id, "guidance")
